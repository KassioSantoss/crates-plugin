...
public class DefaultCrateRenderer implements CrateRendererService {
    ...
    public DefaultCrateRenderer(CacheDependencyResolverService resolverService) {
        this.resolverService = resolverService;
    }

    @Override
    public void renderer(FileConfiguration config) {
        ...
        for (String crateID : crates.getKeys(false)) {
            ...
            String value = config.getString(PATH + crateID + ".animation.value", "0");
            String color = config.getString(PATH + crateID + ".animation.color", "BLACK");
            ...
            showRenderedCrates("&a",
                    ...
                    "&a" + crateID + "&b -> value: &a" + value,
                    "&a" + crateID + "&b -> color: &a" + color,
            ...
            for (String rewardName : rewardsName) {
                Reward reward = resolverService.getRewardCache().findRewardById(rewardName);
            ...
            resolverService.getCrateCache().add(crate.getNameSpace(), crate);
            resolverService.getCrateCache().addNameSpacedToList(crate.getNameSpace());
            resolverService.getCrateCache().addCrateById(crate.getId(), crate);
            resolverService.getRewardCache().addRewardsForCrate(crate, rewardList);
